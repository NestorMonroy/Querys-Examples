...
-- ====================================================================
-- ANÁLISIS: PROMEDIO DE PRODUCTOS - MATRIZ PIVOTEADA CON TOTALES
-- ====================================================================

SELECT 'EJECUTANDO ANÁLISIS: PROMEDIO DE PRODUCTOS - MATRIZ PIVOTEADA' as proceso;

-- Consulta directa sin tabla temporal - Matriz pivoteada con ROLLUP
SELECT 
    COALESCE(categoria, 'TOTAL_GENERAL') as categoria,
    ROUND(AVG(CASE WHEN cIDR = @OP THEN productos_por_trimestre END), 2) as OP,
    ROUND(AVG(CASE WHEN cIDR = @ONa THEN productos_por_trimestre END), 2) as ONa,
    ROUND(AVG(productos_por_trimestre), 2) as PROMEDIO_GENERAL,
...
ORDER BY 
    CASE 
        WHEN categoria IS NULL THEN 1  -- TOTAL al final
        ELSE 0
    END,
