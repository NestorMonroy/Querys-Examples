...
SELECT 
    categoria,
    AVG(productos_por_trimestre) as promedio_productos,
...
FROM (
    ...
    FROM (
        SELECT @Q1_nombre as trimestre, categoria, producto_id
        FROM ventas_1
        ...
        SELECT @Q2_nombre as trimestre, categoria, producto_id
        FROM ventas_2
        ...
        SELECT @Q3_nombre as trimestre, categoria, producto_id
        FROM ventas_3
        WHERE fechaTabla >= @Q3_inicio AND fechaTabla <= @Q3_fin
        AND cIDR IN (@OP, @ONa)
    ) datos
    GROUP BY categoria, trimestre
) productos_categoria_trimestre
GROUP BY categoria
