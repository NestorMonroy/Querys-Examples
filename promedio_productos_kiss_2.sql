...
/*********************************************************************************************
     Script          : Análisis 2 - Promedio de Productos por cIDR (KISS)
     
     Create          : AGOSTO/2025
     Engine          : MariaDB/MySQL
     
     Parámetros Variables:
     @OP, @ONa - Códigos de organización (comparación entre ambos)
     ...
     - Comparación @OP (123) vs @ONa (369) por categoría
...
-- ====================================================================
-- ANÁLISIS: PROMEDIO DE PRODUCTOS POR CATEGORÍA Y cIDR
-- ====================================================================

SELECT 'EJECUTANDO ANÁLISIS: PROMEDIO DE PRODUCTOS @OP vs @ONa POR CATEGORÍA' as proceso;

-- Consulta directa sin tabla temporal - Comparación OP vs ONa
SELECT 
    categoria,
    AVG(CASE WHEN cIDR = @OP THEN productos_por_trimestre END) as promedio_productos_OP,
    AVG(CASE WHEN cIDR = @ONa THEN productos_por_trimestre END) as promedio_productos_ONa,
    AVG(productos_por_trimestre) as promedio_productos_total,
    COUNT(DISTINCT CASE WHEN cIDR = @OP THEN trimestre END) as trimestres_OP,
    COUNT(DISTINCT CASE WHEN cIDR = @ONa THEN trimestre END) as trimestres_ONa
FROM (
    SELECT 
        categoria,
        cIDR,
...
GROUP BY categoria
ORDER BY promedio_productos_total DESC;

-- ====================================================================
-- FINALIZACIÓN
-- ====================================================================

SELECT '<<< ANÁLISIS PROMEDIO DE PRODUCTOS POR cIDR COMPLETADO >>>' as resultado;
