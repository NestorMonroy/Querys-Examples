-- ====================================================================
-- INFORMACIÓN DEL ENTORNO
-- ====================================================================

SELECT 'EJECUTADO EN:' as info;

SELECT 
    DATABASE() as 'Base de Datos'
    , USER() as 'Usuario'
    , @@version as 'Versión MySQL/MariaDB'
FROM DUAL;

SELECT NOW() as 'FECHA DE INICIO';

/*********************************************************************************************
     Script          : Análisis 2 - Promedio de Productos con GROUP BY + COUNT (Eficiente)
     
     Create          : AGOSTO/2025
     Engine          : MariaDB/MySQL
     
     Parámetros Variables:
     @OP, @ONa - Códigos de organización
     @Q1_inicio, @Q1_fin - Rango de fechas Q1 2025
     @Q2_inicio, @Q2_fin - Rango de fechas Q2 2025  
     @Q3_inicio, @Q3_fin - Rango de fechas Q3 2025
     
     Notas:
     - GROUP BY + COUNT(*) en lugar de COUNT(DISTINCT) - más eficiente
     - DISTINCT aplicado solo donde es necesario
     - KISS: Keep It Simple, Stupid
     - Optimizado para grandes volúmenes
     
*********************************************************************************************/

-- ====================================================================
-- CONFIGURACIÓN DE VARIABLES
-- ====================================================================

SET @OP = 123;
SET @ONa = 369;

SET @Q1_nombre = 'Q01_25';
SET @Q1_inicio = '2025-01-01';
SET @Q1_fin = '2025-03-31';

SET @Q2_nombre = 'Q02_25';
SET @Q2_inicio = '2025-04-01';
SET @Q2_fin = '2025-06-30';

SET @Q3_nombre = 'Q03_25';
SET @Q3_inicio = '2025-07-01';
SET @Q3_fin = '2025-09-30';

-- Mostrar configuración
SELECT 
    'CONFIGURACIÓN DE VARIABLES' as seccion
    , @OP
    , @ONa 
    , @Q1_nombre
    , @Q1_inicio
    , @Q1_fin
    , @Q2_nombre
    , @Q2_inicio
    , @Q2_fin
    , @Q3_nombre
    , @Q3_inicio
    , @Q3_fin;

-- ====================================================================
-- ANÁLISIS: PROMEDIO DE PRODUCTOS CON GROUP BY + COUNT (EFICIENTE)
-- ====================================================================

SELECT 'EJECUTANDO ANÁLISIS: PROMEDIO DE PRODUCTOS - GROUP BY + COUNT EFICIENTE' as proceso;

-- Técnica eficiente: GROUP BY + COUNT(*) en lugar de COUNT(DISTINCT)
SELECT 
    CASE 
        WHEN cIDR = @OP THEN 'OP'
        WHEN cIDR = @ONa THEN 'ONa'
    END as organizacion,
    categoria,
    ROUND(AVG(productos_unicos), 2) as promedio_productos
FROM (
    -- Paso 2: Contar productos únicos por grupo (más eficiente que COUNT DISTINCT)
    SELECT 
        cIDR,
        categoria,
        trimestre,
        COUNT(*) as productos_unicos  -- COUNT(*) sobre datos ya deduplicados
    FROM (
        -- Paso 1: Eliminar duplicados con DISTINCT (aplicado solo una vez)
        SELECT DISTINCT 
            cIDR,
            categoria,
            trimestre,
            producto_id
        FROM (
            SELECT @Q1_nombre as trimestre, cIDR, categoria, producto_id
            FROM ventas_1
            WHERE fechaTabla >= @Q1_inicio AND fechaTabla <= @Q1_fin
            AND cIDR IN (@OP, @ONa)

            UNION ALL

            SELECT @Q2_nombre as trimestre, cIDR, categoria, producto_id
            FROM ventas_2
            WHERE fechaTabla >= @Q2_inicio AND fechaTabla <= @Q2_fin
            AND cIDR IN (@OP, @ONa)

            UNION ALL

            SELECT @Q3_nombre as trimestre, cIDR, categoria, producto_id
            FROM ventas_3
            WHERE fechaTabla >= @Q3_inicio AND fechaTabla <= @Q3_fin
            AND cIDR IN (@OP, @ONa)
        ) datos_consolidados
    ) productos_deduplicados
    GROUP BY cIDR, categoria, trimestre
) conteos_por_trimestre
GROUP BY cIDR, categoria
ORDER BY 
    CASE WHEN cIDR = @OP THEN 1 ELSE 2 END,
    promedio_productos DESC;

-- ====================================================================
-- FINALIZACIÓN
-- ====================================================================

SELECT '<<< ANÁLISIS PROMEDIO DE PRODUCTOS COMPLETADO >>>' as resultado;

SELECT 
    'PROCESO COMPLETADO' as evento,
    'GROUP_BY_COUNT_EFICIENTE' as tecnica_utilizada,
    NOW() as timestamp_fin
FROM DUAL;