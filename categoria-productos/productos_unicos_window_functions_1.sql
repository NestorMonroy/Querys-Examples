...
/*********************************************************************************************
     Script          : Análisis 1 - Productos Únicos con Window Functions (KISS)
     ...
     Notas:
     - Window Functions en lugar de COUNT(DISTINCT) - más eficiente
     - KISS: Keep It Simple, Stupid
     - Una sola consulta principal
...
-- ====================================================================
-- ANÁLISIS: PRODUCTOS ÚNICOS CON WINDOW FUNCTIONS (KISS)
-- ====================================================================

SELECT 'EJECUTANDO ANÁLISIS: PRODUCTOS ÚNICOS - WINDOW FUNCTIONS KISS' as proceso;

-- Consulta simple con Window Functions para eliminar duplicados
SELECT 
    CASE 
...
FROM (
    SELECT 
        ...
        trimestre,
        producto_id,
        ROW_NUMBER() OVER (PARTITION BY cIDR, trimestre, producto_id ORDER BY fechaTabla) as rn
    FROM (
...
WHERE rn = 1  -- Solo primera aparición
